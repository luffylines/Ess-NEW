...
<div class="container-fluid">
    ...
    <div class="card shadow-sm">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <form action="{{ route('hr.bulk-approve') }}" method="POST" onsubmit="return confirm('Are you sure you want to approve ALL pending attendance records?')">
                @csrf
                <input type="hidden" name="attendance_ids" value="{{ $pendingAttendances->pluck('id')->join(',') }}">
                <button type="submit" class="btn btn-success btn-sm">
                    <i class="fas fa-check-double me-1"></i>Bulk Approve All
                </button>
            </form>
            @endif
        </div>
