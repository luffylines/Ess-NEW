          ...
          <td>
            <div class="form-check form-switch">
              ...
              <label class="form-check-label" for="store-{{ $store->id }}">
                <span class="status-text">{{ $store->active ? 'Active' : 'Inactive' }}</span>
              </label>
            </div>
          </td>
          <td class="text-end">
            <a href="{{ route('admin.stores.edit', $store) }}" class="btn btn-sm btn-outline-primary">Edit</a>
            <form action="{{ route('admin.stores.destroy', $store) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this store?')">
              @csrf
              @method('DELETE')
